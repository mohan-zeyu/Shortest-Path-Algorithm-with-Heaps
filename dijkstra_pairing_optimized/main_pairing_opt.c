...
int main() {
    ...
    WorkSpacePairing* workspace = (WorkSpacePairing*)malloc(sizeof(WorkSpacePairing));
    workspace->distance = (int*)malloc(sizeof(int) * graph->num_vertices);
    workspace->visited = (bool*)malloc(sizeof(bool) * graph->num_vertices);
    workspace->heap = createPairingHeap(graph->num_vertices);
    
    // Bidirectional workspace
    BiDirWorkSpacePairing* bidir_workspace = (BiDirWorkSpacePairing*)malloc(sizeof(BiDirWorkSpacePairing));
    
    bidir_workspace->forward = (WorkSpacePairing*)malloc(sizeof(WorkSpacePairing));
    ...
    bidir_workspace->forward->heap = createPairingHeap(graph->num_vertices);
    
    bidir_workspace->backward = (WorkSpacePairing*)malloc(sizeof(WorkSpacePairing));
    ...
    bidir_workspace->backward->heap = createPairingHeap(graph->num_vertices);
    ...
    if (!filein || !fileout) {
        ...
        for (int i = 0; i < query_count; i++) {
            results[i] = BidirectionalDijkstraPairing(bidir_workspace, graph, reverse_graph, 
